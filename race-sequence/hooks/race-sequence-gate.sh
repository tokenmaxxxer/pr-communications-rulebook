...
#
# PreToolUse (Write|Edit|MultiEdit|Bash) gate enforcing RACE order
# (Research -> Action -> Communication -> Evaluation) on terminal
...
#
# Kill switch: RACE_SEQUENCE_GATE_DISABLE=1 (or true/yes/on) skips the gate
...
. "${CLAUDE_PLUGIN_ROOT_CORE:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../../core" && pwd -P)}/hooks/lib/gate-lib.sh" \
  || { echo "race-sequence-gate.sh: cannot source gate-lib.sh" >&2; exit 2; }
gate_trap_fail_closed
set -uo pipefail

GATE_NAME="race-sequence-gate"
...
try:
    ...
    m = re.search(r'^## Communications plan\s*$', new_text, re.M)
    if not m:
        deny("missing '## Communications plan' section")
    ...
    LABEL_RE = re.compile(r'^\*\*(Research|Action|Communication|Evaluation)\*\*\s*:?\s*$', re.M)

    lines = section.split("\n")
    line_no_of = {}
    for label in ("Research", "Action", "Communication", "Evaluation"):
        line_no_of[label] = None
    offset = 0
    for i, line in enumerate(lines):
        m_label = re.match(r'^\*\*(Research|Action|Communication|Evaluation)\*\*\s*:?\s*$', line)
        if m_label:
            label = m_label.group(1)
            if line_no_of[label] is None:
                line_no_of[label] = i

    missing = [l for l in ("Research", "Action", "Communication", "Evaluation") if line_no_of[l] is None]
    if missing:
        deny("missing top-level **%s** label(s) inside '## Communications plan' "
             "(a bolded word in prose does not count — it must start its own "
             "line as **Label** or **Label**:)" % "**/**".join(missing))

    order = ["Research", "Action", "Communication", "Evaluation"]
    for before, after in zip(order, order[1:]):
        if line_no_of[after] <= line_no_of[before]:
            deny("%s appears before %s — success criteria defined after send "
                 "is RACE's core failure mode" % (after, before))

    sys.exit(0)
except SystemExit:
    raise
except Exception as _fc_e:  # fail-closed-on-internal-error
    _fc_sys.stderr.write("race-sequence-gate: fail-closed: internal error: %r\n" % (_fc_e,))
    _fc_sys.exit(2)
PY
